...
public class ResourceEntry
{
  ...
  private static final Map<String, Integer> extensionTypes = buildExtensionTypes();
  private static final Map<Integer, String> resourceExtensions = buildResourceExtensions();

  private static Map<String, Integer> buildExtensionTypes() {
    Map<String, Integer> map = new HashMap<>(fileExtensions.length * 2);
    for (int i = 0; i < fileExtensions.length; i++) {
      map.put(fileExtensions[i], resourceTypes[i]);
    }
    return map;
  }

  private static Map<Integer, String> buildResourceExtensions() {
    Map<Integer, String> map = new HashMap<>(resourceTypes.length * 2);
    for (int i = 0; i < resourceTypes.length; i++) {
      map.put(resourceTypes[i], fileExtensions[i]);
    }
    return map;
  }
    ...
    throws DBException
  {
    ...
    Integer type = extensionTypes.get(ext);
    if (type == null)
    ...
    throws DBException
  {
    ...
    String ext = resourceExtensions.get(resourceType);
    if (ext == null) {
