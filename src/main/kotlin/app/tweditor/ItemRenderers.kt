...
class ItemListCellRenderer(private val environment: AppEnvironment, private val iconSize: Int) :
    ...
    private fun itemIcon(item: InventoryItem): Icon? {
        return try {
            environment.icons.itemIcon(item.element.getValue() as DBList, iconSize)
...
class ItemTreeCellRenderer(private val environment: AppEnvironment, private val iconSize: Int) :
    ...
    private fun templateIcon(template: ItemTemplate): Icon? =
        environment.icons.templateIcon(template, iconSize)
}
