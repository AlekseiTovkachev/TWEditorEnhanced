...
class IconLibrary(private val environment: AppEnvironment) {
    var lateIconListener: Runnable? = null
    ...
    private val baseItemClasses = ConcurrentHashMap<Int, Pair<String, String>>()
    ...
    fun itemIcon(fields: DBList, size: Int): ImageIcon? {
        return scaledIcon(itemIconResref(fields) ?: return null, size)
    }

    fun templateIcon(template: ItemTemplate, size: Int): ImageIcon? {
    ...
    private fun resolveIconResref(baseItem: Int, modelPart: Int, templateResRef: String): String? {
        val (itemClass, defaultModel) = baseItemClasses[baseItem] ?: ("" to "")
    ...
    private fun scaledIcon(resref: String, size: Int): ImageIcon? {
        ...
        val icon = ImageIcon(scaleBicubic(image, size))
    ...
    private fun scaleBicubic(image: BufferedImage, size: Int): BufferedImage {
        val aspect = image.width.toDouble() / image.height.toDouble()
        val minExtent = size / 3
        val drawWidth: Int
        val drawHeight: Int
        if (aspect >= 1.0) {
            drawWidth = size
            drawHeight = (size / aspect).toInt().coerceAtLeast(minExtent)
        } else {
            drawHeight = size
            drawWidth = (size * aspect).toInt().coerceAtLeast(minExtent)
        }
            ...
            if (input != null) {
                try {
                    ...
                    for (row in 0 until table.getResourceCount()) {
                        baseItemClasses[row] = table.getString(row, "ItemClass") to table.getString(row, "DefaultModel")
                    }
                } finally {
                    input.close()
                }
